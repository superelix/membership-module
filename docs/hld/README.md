...
|---|---|---|---|
| **Plans** | `MONTHLY`, `YEARLY` seeded via `CommandLineRunner` (MP-PLAN-01 read-only) | Admin CRUD/lifecycle APIs (MP-PLAN-02/03), `QUARTERLY` | Concurrent-edit `409` (MP-PLAN-EDGE-06 / MP-AC-049) |
| **Tiers/criteria** | `SILVER`/`GOLD`/`PLATINUM` seeded; **`ORDER_COUNT_MIN` only**, wired through the full `TierCriterionEvaluator` strategy + registry (not hardcoded) | `ORDER_VALUE_MIN`, `COHORT_MEMBERSHIP` added as pure-addition evaluators (the extensibility proof, see LLD-02 §6); admin criteria API (MP-API-12) | `ALL`/`ANY` admin combinator changes at scale, rank-collision validation polish |
...
| **Subscription lifecycle** | Subscribe, cancel, plan upgrade/downgrade (deferred-to-boundary, no proration), current-membership read | — | Auto-renew job, `PaymentStub` grace period, `PAYMENT_FAILED`→`EXPIRED` (MP-SUB-06) |
...
| **Admin surface** | none (seed data only) | Plan/tier/criteria/benefit admin APIs | Member cohort-assignment API (MP-API-15) |
| **Entities** | `Member`, `Plan`, `Subscription`, `MembershipStatus`, `Tier`, `TierCriteriaSet`, `TierCriterion`, `TierBenefit`, `Order`, `OrderItem`, `TierChangeLog`, `IdempotencyRecord` (11) | `Deal` | — (no new entities; `BenefitSnapshot` is a JSON column on `Order`, never a separate table — see LLD-01) |
...
| Scheduling | Spring `@Scheduled` (single JVM, no distributed scheduler) | Matches single-instance assumption (PRD 08 §1); trivial to swap for ShedLock/Quartz if the deployment ever scales out — not built, since not needed for Day-1 or the stated scope. |
